...
public class ColorAPI {
        ...
        while(true) {
            ...
            do {
                do {
                    ...
                    String var10000 = matcher.group(2);
                    colorOne = getColor("{#" + var10000.replace("#", "") + "}");
                    var10000 = matcher.group(5);
                    colorTwo = getColor("{#" + var10000.replace("#", "") + "}");
                } while(colorOne == null);
            } while(colorTwo == null);
            ...
            for(int i = 0; i < var5.length(); ++i) {
                ...
                if (!formatsSet.isEmpty()) {
                    Iterator var15 = formatsSet.iterator();

                    while(var15.hasNext()) {
                        ColorAPI format = (ColorAPI)var15.next();
        ...
        if (input == null) {
            ...
            if (!input.contains("{#")) {
                ...
                for(Matcher matcher = x1.matcher(input); matcher.find(); input = input.replace(str, stringBuilder.toString())) {
                    ...
                    char[] var5 = hexCharArray;
                    int var6 = hexCharArray.length;

                    for(int var7 = 0; var7 < var6; ++var7) {
                        char value = var5[var7];
    ...
    public static TextComponent translateTextComponent(String input) {
        if (input == null) {
            return null;
        } else {
            input = processGradient(input);
    ...
    public static TextComponent colorizeTextComponent(String input) {
        return input == null ? null : translateTextComponent(String.valueOf(input));
    }
    ...
    public static String deColorize(String input, boolean translate) {
        if (input == null) {
            ...
            String replacedStr;
            if (input.contains("&x")) {
                for(Matcher matcher = x3.matcher(input); matcher.find(); input = input.replace(matcher.group(), "{#" + replacedStr + "}")) {
                    replacedStr = matcher.group(3).replace("&", "");
                }
            }
    ...
    public static ColorAPI getFormat(String input) {
        if (input == null) {
            ...
            if (deColorizedStr.length() > 1 && String.valueOf(deColorizedStr.charAt(deColorizedStr.length() - 2)).equalsIgnoreCase("&")) {
                ...
                Iterator var2 = COLORS_BY_CHAR.entrySet().iterator();

                while(var2.hasNext()) {
                    Entry<Character, ColorAPI> characterChatColorAPIEntry = (Entry)var2.next();
                    if (String.valueOf(characterChatColorAPIEntry.getKey()).equalsIgnoreCase(input)) {
                        return ((ColorAPI)characterChatColorAPIEntry.getValue()).isFormat() ? (ColorAPI)characterChatColorAPIEntry.getValue() : null;
                    }
                }
            }
    ...
    public static ColorAPI getColor(String input) {
        if (input == null) {
            ...
            if (decolorizedStr.length() > 1 && String.valueOf(decolorizedStr.charAt(decolorizedStr.length() - 2)).equalsIgnoreCase("&")) {
                input = input.substring(input.length() - 1);
                Iterator var4 = COLORS_BY_CHAR.entrySet().iterator();

                while(var4.hasNext()) {
                    Entry<Character, ColorAPI> characterChatColorAPIEntry = (Entry)var4.next();
                    if (String.valueOf(characterChatColorAPIEntry.getKey()).equalsIgnoreCase(input)) {
                        return (ColorAPI)characterChatColorAPIEntry.getValue();
                    }
                }
            }
